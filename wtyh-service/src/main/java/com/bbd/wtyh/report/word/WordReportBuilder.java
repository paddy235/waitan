...
/**
 * word 版报告生成类 Created by cgj on 2017/5/26.
 */
...
public class WordReportBuilder {
	...
	/**
	 * 正文部件关系 mainRelationPart.removePart(PartName partName); 删除已存的部件
	 ...
	 *             fileE
	 */
	public WordReportBuilder(String reportTemplateFileName, ReportType reportType) throws Docx4JException, FileNotFoundException {
		...
		// List<Relationship> mainRelationList =
		// mainRelationPart.getRelationships().getRelationship(); //这个暂时没啥用
	...
	public static void main(String[] argc) { // 文档生成类使用示例
		...
		try {
			...
			wrb.setCompanySummary(companyName, new ArrayList<String>() {
			...
			}, null, new ArrayList<List<String>>() {
			...
			}, null, null, new ArrayList<List<String>>() {
			...
			wrb.setRelatedPartyMappingInfo(byt, byt, null, null, null);
	...
	/**
	 * 注意：这个仅线下理财才有 设置动态风险表：选取范围为最近一年，如果时间不足一年，则选取一年内最远的两个时间点。
	 * 
	 * @param dynamicRiskTable
	 *            map类型，key值包含最左这一列的所有项目，其中“动态风险指数”这一行特殊处理： {key："动态风险指数",
	 *            value: "[日期区间]"}，{key："动态风险指数^r1", value: "[指数数值]"} 日期区间
	 *            样式“2016.3-2017.3”
	 */
	public void setDynamicRiskTable(Map<String, String> dynamicRiskTable) {
		...
		StringBuffer thisErrRecord = new StringBuffer();
		setTableSecondColumn(mainParagraphList, 0, "fengxianxinxi_dongtaifengxianbiao", dynamicRiskTable, 0, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	...
	public void delPlatPFxxTable() {
		...
		if (idx >= 0 || (idx + 2) < mainParagraphList.size()) {
			idx++; // 将序号指向可能的表格对象
			if (mainParagraphList.get(idx) instanceof JAXBElement && ((JAXBElement) mainParagraphList.get(idx)).getValue() instanceof Tbl) {
	...
	/**
	 * 注意：这个仅网络借贷才有 添加平台信息（涉及模板中的平台评分的一行和第二章全部内容），调用一次，加入一个平台的信息，可多次调用加入多个。
	 * 
	 * @param gradeInfo
	 *            平台评分信息，这里面的key:"平台名称" 是必传参数
	 * @param coreData
	 *            平台核心数据
	 * @param transferQuantityTrend
	 *            平台交易量走势
	 * @param interestRateTrend
	 *            平台利率走势
	 * @param loanBalance
	 *            平台贷款余额
	 ...
	 *            平台舆情
	 */
	public int addPlatInfo(Map<String, String> gradeInfo, Map<String, String> coreData, List<List<String>> transferQuantityTrend,
			List<List<String>> interestRateTrend, List<List<String>> loanBalance, List<List<String>> publicSentiment) {
		...
		// 设置 平台利率走势
		this1ErrRecord = new StringBuffer();
		addTableRows(newBlock, 0, "pingtai_lilvzoushi", interestRateTrend, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 设置 平台贷款余额
		this1ErrRecord = new StringBuffer();
		addTableRows(newBlock, 0, "pingtai_daikuanyu_e", loanBalance, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 设置 平台舆情
		this1ErrRecord = new StringBuffer();
		addTableRows(newBlock, 0, "pingtai_pingtaiyuqing", publicSentiment, 99, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
	...
	public void setCompanyBaseInfoTable(Map<String, String> baseInfoTable) {
		StringBuffer thisErrRecord = new StringBuffer();
		setTableSecondColumn(mainParagraphList, 0, "qixin_jibengxinxi", baseInfoTable, 0, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	 ...
	 * 设置企业股东信息
	 * 
	 * @param stockholderInfo
	 *            List<List<"股东","股东类型">>
	 */
	public void setStockholderInfo(List<List<String>> stockholderInfo) {
		StringBuffer thisErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_gudongxinxi", stockholderInfo, Integer.MAX_VALUE, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	 ...
	 * 设置企业出资信息
	 * 
	 * @param stockholderContributionInfo
	 *            List<List<"股东","认缴出资额(万元)", "认缴出资时间", ...>>
	 */
	public void setStockholderContributionInfo(List<List<String>> stockholderContributionInfo) {
		StringBuffer thisErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_gudongchuzi", stockholderContributionInfo, Integer.MAX_VALUE, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	 ...
	 * 设置董监高信息
	 * 
	 * @param trusteeSupervisorSeniorInfo
	 *            List<List<"姓名","职务">>
	 */
	public void setTrusteeSupervisorSeniorInfo(List<List<String>> trusteeSupervisorSeniorInfo) {
		StringBuffer thisErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_dongjiangaoxinxi", trusteeSupervisorSeniorInfo, Integer.MAX_VALUE, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	 ...
	 * 
	 * @param changeInfo
	 *            List<List<"变更事项", "变更前", "变更后", "变更时间">>
	 */
	public void setCompanyChangeInfo(List<List<String>> changeInfo) {
		StringBuffer thisErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_biangengxinxi", changeInfo, Integer.MAX_VALUE, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	 ...
	 * 设置企业关联方图谱信息
	 * 
	 * @param relatedPartyMapping
	 *            总的关联方图谱 //ByteArrayOutputStream aa =new
	 *            ByteArrayOutputStream();
	 * @param oneDegreeMapping
	 *            一度关联图
	 * @param oneDegreeDistribute
	 *            一度关联企业行业分布列表
	 * @param twoDegreeMapping
	 *            二度关联图
	 * @param twoDegreeDistribute
	 *            二度关联企业行业分布列表
	 */
	public void setRelatedPartyMappingInfo(byte[] relatedPartyMapping, byte[] oneDegreeMapping, List<List<String>> oneDegreeDistribute,
		...
		// 一度关联图
		this1ErrRecord = new StringBuffer();
		replaceImage(mainParagraphList, 0, "qixin_yidutupu", oneDegreeMapping, null, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 一度关联表
		this1ErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_yidubiaoge", oneDegreeDistribute, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 二度关联图
		this1ErrRecord = new StringBuffer();
		replaceImage(mainParagraphList, 0, "qixin_edutupian", twoDegreeMapping, null, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 二度关联表
		this1ErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_edubiaoge", twoDegreeDistribute, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
	 ...
	 * 
	 * @param recruitInfoList
	 *            招聘信息列表
	 * @param recruitPeopleDistribute
	 *            招聘人员分布
	 * @param recruitPeopleSalary
	 *            薪酬分布
	 */
	public void setRecruitInfo(List<List<String>> recruitInfoList, Map<String, String> recruitPeopleDistribute,
		...
		MapSort_funIf1 mSort = (Map<String, String> src) -> { // 升序排序
			...
			Double sum = 0D;
			/*
			 * for (Map.Entry<String, String> entry : src.entrySet()) { Double
			 * keyDb; try { keyDb =Double.valueOf(entry.getValue()); } catch (
			 * Exception e) { keyDb = 0D; } sum +=keyDb; //求和 int iIdx =0; for (
			 * ; iIdx <rpLst.size(); iIdx++ ) { //搜索插入点 Map.Entry<Double,
			 * String> entry1 =rpLst.get(iIdx); if( keyDb <entry1.getKey() ) {
			 * break; } } rpLst.add( iIdx, new AbstractMap.SimpleEntry<>( keyDb,
			 * entry.getKey() ) ); }
			 */
		...
		// 设置招聘信息列表
		int aRst = addTableRows(mainParagraphList, 0, "qixin_zhaopin_xinxi", recruitInfoList, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		if (recruitPeopleDistribute != null && recruitPeopleDistribute.size() > 0) {
			...
			// 替换描述文字
			DocxUtils.replacePlaceholder(mainParagraphList, new HashMap<String, String>() {

				{
					put("$$企业信息-招聘人员分布描述", peopleDistDes.toString());
				}
			});
			// 更新图表数据
		...
		if (recruitPeopleSalary != null && recruitPeopleSalary.size() > 0) {
			...
			TextSynthesis_funIf1 pS = (StringBuffer outStr, Map.Entry<Double, String> entry, String additory, String endStr) -> {
				BigDecimal perVal = BigDecimal.valueOf(entry.getKey());
				perVal = perVal.multiply(new BigDecimal(100));
				// perVal =perVal.divide( new BigDecimal(total),2,
			...
			DocxUtils.replacePlaceholder(mainParagraphList, new HashMap<String, String>() {
				...
				{
					put("$$企业信息-薪酬分布描述", peopleSalaryDes.toString());
				}
			});
			// 更新图表数据
			List<String> rIdList = new ArrayList<>();
	 ...
	 * 
	 * @param noCreditDebtor
	 *            失信被执行人
	 * @param debtor
	 *            被执行人
	 * @param judgeDoc
	 *            裁判文书
	 * @param courtAnnouncement
	 *            法院公告
	 * @param openCourt
	 *            开庭公告
	 */
	public void setCompanyLawsuitInfo(List<List<String>> noCreditDebtor, List<List<String>> debtor, List<List<String>> judgeDoc,
			List<List<String>> courtAnnouncement, List<List<String>> openCourt) {
		StringBuffer thisErrRecord = new StringBuffer();
		StringBuffer this1ErrRecord = new StringBuffer();

		// 设置失信被执行人
		// this1ErrRecord =new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_susong_shixinbei", noCreditDebtor, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 设置被执行人
		this1ErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_susong_beizhixing", debtor, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 裁判文书
		this1ErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_susong_caiwen", judgeDoc, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 法院公告
		this1ErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_susong_fagao", courtAnnouncement, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
		...
		// 开庭公告
		this1ErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_susong_kaigao", openCourt, Integer.MAX_VALUE, this1ErrRecord);
		if (this1ErrRecord.length() > 0) {
	...
	public void setCompanyPatentInfo(List<List<String>> patentInfo) {
		StringBuffer thisErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_zhuanlixinxi", patentInfo, Integer.MAX_VALUE, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	...
	public void setCompanyPublicSentimentInfo(List<List<String>> publicSentiment) {
		StringBuffer thisErrRecord = new StringBuffer();
		addTableRows(mainParagraphList, 0, "qixin_qiyeyuqing", publicSentiment, Integer.MAX_VALUE, thisErrRecord);
		if (thisErrRecord.length() > 0) {
	...
	public void delTempFile() throws IOException {
		...
		// String tempPath =(new File(AccessController.doPrivileged(new
		// GetPropertyAction("java.io.tmpdir")))).getPath();
	 ...
	 *             E
	 */
	private void addDataToChartById(List<String> rIdList, Map<String, String> dataMap, StringBuffer thisErrRecord) throws Exception {
				...
				for (Relationship rs : chart.getRelationshipsPart().getRelationships().getRelationship()) {
					...
					if (tgStr.contains("xlsx")) {
						...
						if (cPt instanceof EmbeddedPackagePart) {
							...
							/*
							 * OpcPackage objEmb = embPP.extractPackage(); if(
							 * objEmb instanceof SpreadsheetMLPackage ) {
							 * //SpreadsheetMLPackage xlsxPkg =
							 * (SpreadsheetMLPackage) objEmb; for (Object
							 * shtSart :
							 * ((SpreadsheetMLPackage)objEmb).getParts().
							 * getParts().values() ) { if( shtSart instanceof
							 * WorksheetPart ) { Worksheet ws
							 * =((WorksheetPart)shtSart).getContents();
							 * SheetData data = ws.getSheetData();
							 * //data.getRow().clear(); for( Row row :
							 * data.getRow() ) { for (Cell cell :
							 * ((Row)row).getC() ) { if
							 * (cell.getT().equals(STCellType.N)) { Object to
							 * =cell.getV(); //cell.setV("0.444"); } } } } } }
							 * objEmb.save( tmpBaos );
							 */
							...
							// embPP.getBuffer().clear();embPP.getBuffer().put(tmpBaos.toByteArray());
							// //这种方式更新数据是错误的
						}
						// chart.getRelationshipsPart().removePart(cPt.getPartName());
						// //不能这样删除内嵌的xlsx，删除后目标文件无法打开
						break;
					}
				}
				if (mapIdx < 1) {
	 ...
	 *            单元格地址，如“C12”
	 * @param cellType
	 *            内容属性STCellType.INLINE_STR为字符串、其他则为数值型
	 * @return 返回创建好的单元格对象
	 */
	 ...
	 */
	private int searchPartRelationIdFromParagraphList(List<Object> myParagraphList, int startIdx, String anchor, List<String> rIdList,
			StringBuffer thisErrRecord) {
	 ...
	 *            "##@ah-" 的后缀
	 * @param srcBytes
	 *            存放图片的字节数组，注意：若此参数为空者使用“filePathName”参数指定的图片文件作为替换源
	 * @param srcFile
	 *            “srcBytes”为空时，此参数必须有效
	 * @param thisErrRecord
	 *            出错记录
	 * @return 返回表格所在的段落标号
	 */
	private int replaceImage(List<Object> myParagraphList, int startIdx, String anchor, byte[] srcBytes, File srcFile,
			StringBuffer thisErrRecord) {
		int idx = searchAnchorFromParagraphListToIndex(myParagraphList, startIdx, anchor);
		idx++;
		List drawingList = DocxUtils.getAllElementFromObject(myParagraphList.get(idx), Drawing.class);
		...
		try {
			...
			BinaryPartAbstractImage aImg = (null != srcBytes ? BinaryPartAbstractImage.createImagePart(wmp, srcBytes)
	 ...
	 */
	private int addTableRows(List<Object> paragraphList, int startIdx, String anchor, List<List<String>> rowsData, int maxRowNum,
			StringBuffer thisErrRecord) {
		...
		idx++; // 将序号指向可能的表格对象
		if (paragraphList.get(idx) instanceof JAXBElement && ((JAXBElement) paragraphList.get(idx)).getValue() instanceof Tbl) {
	...
	/**
	 * 用于四段落文档片段（"无相关信息（尚未查询到目标公司）"、定位符、表格(图片)、备注）选择性删除定位符前或后， 且当片段结构完整才删除指定部分
	 * 
	 * @param front
	 *            删除定位符前（true）或后（false）
	 * @param myParagraphList
	 *            待操作的列表
	 * @param startIdx
	 *            搜索的起始位置
	 ...
	 *            定位符
	 */
	private void delTableOrParagraphFromParagraphList(boolean front, List<Object> myParagraphList, int startIdx, String anchor) {
		int idx = searchAnchorFromParagraphListToIndex(myParagraphList, startIdx, anchor);
		...
		if ((myParagraphList.get(idx - 1) instanceof P)) {
			...
			if (StringUtils.isNotBlank(str) && (str.contains("无相关信息") || str.contains("尚未查询到目标公司"))) {
				if (front) {
	 ...
	 * 
	 * @param myParagraphList
	 *            指定待搜索和修改的文档片段（广义的段落列表）
	 * @param startIdx
	 *            指定从段落列表的哪一个开始搜索
	 * @param anchor
	 *            用于对表格定的字符串
	 * @param replaceMap
	 *            待修改的对应行，key的"^r1"后缀用于处理被合并的单元格的表示问题
	 * @param startRow
	 *            指定从表的哪一行开始替换，可用于跳过标题行（从第0行开始计数）
	 * @param thisErrRecord
	 *            错误记录
			...
			if (objT instanceof Tbl) {
				...
				for (; startRow < listCtt.size(); startRow++) {
					...
					if (objR instanceof Tr) { // 是一个行对象
						List<Object> objC = DocxUtils.getAllElementFromObject(objR, Tc.class); // List<Object>
																								// objC
																								// =((Tr)objR).getContent();
						if (objC.size() >= 2) {
	...
	private void xlsxCreate(ByteArrayOutputStream boaos) throws Exception {
		...
		for (int rowIdx = 0; rowIdx < xy.length; rowIdx++) {
			...
			for (int col = 0; col < xy[rowIdx].length; col++) {
				...
				cell.setR("" + ((char) ('A' + col)) + rowNum); // Apple Numbers
																// needs this,
																// or cell
																// content won't
																// show
		...
		pkg.save(boaos);
		FileOutputStream fos = new FileOutputStream("D:\\bbdPrjIj\\wtyh-dv\\wtyh-web\\src\\main\\resources\\docx\\temporary\\emb1.xlsx");
		pkg.save(fos);
		fos.close();
	}
